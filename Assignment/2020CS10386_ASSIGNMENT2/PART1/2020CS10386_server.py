...
class Server:
    def __init__(self):
        ...
        self.broadcastUDP_sockets=[None]*noClients
        ...
        for i in range(noClients):
           ...
           self.broadcastUDP_sockets[i]=socket.socket(family=socket.AF_INET, type=socket.SOCK_DGRAM)
           self.broadcastUDP_sockets[i].bind((ServerIP,Serverportbroadcast+i))
    ...
    def sDatabroad(self,id,data):
        ...
        self.broadcastUDP_sockets[id].sendto(data.encode(),(ServerIP,clientBasePortbroadcast+id))
        while(True):
                
                # print(msg)
            UDPmsg,_=self.broadcastUDP_sockets[id].recvfrom(bufferSize)
            msg=UDPmsg.decode() 
            if("Success" in msg):
                break;
    ...
    def rDatabroad(self,id):
        ...
        while(True):
            # print("Message from server",msg)
            msg,_=self.broadcastUDP_sockets[id].recvfrom(bufferSize)
            msg=msg.decode()
            bytestoSend="Success"
            self.broadcastUDP_sockets[id].sendto(bytestoSend.encode(),(ServerIP,clientBasePortbroadcast+id))
            break   
        return msg
    ...
    def send_initial_chunk(self,id):
        ...
        for i in range(id,self.noChunks,noClients):
            self.sDataUDP(id,str(i))
            self.sDataTCP(id,self.chunks[i])
    ...
    def end_connections(self):
        ...
        for i in range(noClients):
            self.broadcastUDP_sockets[i].close()
